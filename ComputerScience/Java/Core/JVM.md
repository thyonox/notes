...
可以看到在执行第6行虚拟机指令以及执行第9行虚拟机指令时，都是去常量池的#3位置查找，可见两个对象引用地址是相同的。s3 == s4 的结果为true。当然这不是运算符重载，而是编译器的优化，被称为编译器的魔法，实际上是常量的折叠，因为在编译期间已经确定了。
...
8~9 字节，表示常量池长度，00 23 （十进制35） 表示常量池有 #1~#34项，注意 #0 项不计入，也没有值
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668253311060-384ecc8c-46bf-4518-a442-6dc9b4a295d1.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668335296368-4fcb7bea-e68d-41d3-9baa-09b429f7e95e.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668336182785-e334e8cb-5f36-4f1e-a97a-bc766cb611de.png)
...
public class GenericImpl extends Generic<String>{
    @Override
...
这同时解释了为什么匿名内部类引用局部变量时，局部变量必须是 fifinal 的：因为在创建 Candy11$1 对象时，将 x 的值赋值给了 Candy11$1 对象的 val$x 属性，所以 x 不应该再发生变化了，如果变化，那么 val$x 属性没有机会再跟着一起变化
...
- _java_mirror 即 java 的类镜像，例如对 String 来说，就是 String.class，作用是把 klass 暴露给 java 使用 ，相当于class和Klass的桥梁
- _super 即父类
- _fifields 即成员变量
- _methods 即方法
- _constants 即常量池
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1667994241608-44ec9352-5e5d-452d-b88d-818b216126e9.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1667994692959-060a06b8-430a-42cd-8b6f-efed4545356c.png)
...
- 如果 static 变量是 final 的，但属于引用类型，那么赋值是会在初始化阶段完成
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1667996895097-89eaf77c-c5a5-4f17-b233-3601bbbe10a3.png)

### 4.5.1 Bootstrap ClassLoader
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1667997218162-b07a1078-1b10-4137-8f7b-987ea33169f7.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1667998318103-42227041-7039-4218-855d-4c426b3f954d.png)

将 jar 包拷贝到 JAVA_HOME/jre/lib/ext 重新执行 Load5_2 输出

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1667998354105-49911bfa-cc6e-4cf2-8ff7-31d97af91089.png)
...
6. sun.misc.Launcher$ExtClassLoader // 4 处，调用自己的 fifindClass 方法，是在JAVA_HOME/jre/lib/ext 下找 H 这个类，显然没有，回到 sun.misc.Launcher$AppClassLoader的 // 2 处
...
打印 null，表示它的类加载器是 Bootstrap ClassLoader，会到 JAVA_HOME/jre/lib 下搜索类，但JAVA_HOME/jre/lib 下显然没有 mysql-connector-java-5.1.47.jar 包，这样问题来了，在DriverManager 的静态代码块中，怎么能正确加载 com.mysql.jdbc.Driver 呢？继续看 loadInitialDrivers() 方法：
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668000571558-9ccba95d-259c-4079-8fa3-58205d204342.png)
...
3. 注意不是重写 loadClass 方法，否则不会走双亲委派机制

4. 读取类文件的字节码
5. 调用父类的 defifineClass 方法来加载类
...
对于占据大部分的不常用的代码，我们无需耗费时间将其编译成机器码，而是采取解释执行的方式运行；另一方面，对于仅占据小部分的热点代码，我们则可以将其直接编译成机器码，以达到理想的运行速度。 执行效率上简单比较一下 Interpreter < C1 < C2，总的目标是发现热点代码（hotspot名称的由来），优化之
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668049660739-488becef-30b2-416c-8b98-1508b80f4670.png)
...
**java 内存模型** 是 **Java Memory Model（JMM）**的意思，不等于 java 内存结构。

- JVM内存结构和Java虚拟机运行时区域相关，定义了JVM在运行时如何区分存储程序的数据
- JMM内存模型是和Java并发编程相关，抽象了线程和主内存的关系，规定了从 Java 源代码到 CPU 可执行指令的这个转化过程要遵守哪些和并发相关的原则和规范

简单的说，JMM 定义了一套在**多线程读写共享数据**时（成员变量、数组）时，对数据的**可见性**、**有序性**、和**原子性**的规则和保障，其主要目的是为了简化多线程编程，增强程序可移植性的。

**操作系统内存模型：**

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668130426563-b80bae3c-fd76-49b6-b142-4998a4fb29af.png)

**Java内存模型：**

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668131832507-876af9f0-0f19-4e2d-bfda-042c290c2de1.png)

注意：上图存在一些问题，线程与主存之间存在高速缓存，用于缓存共享变量
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668070703984-6f14dffd-8914-4e6c-832e-532ec79398e1.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668071680237-8dbf10e6-28b3-4510-b66f-bf424f6c2708.png)

2. 因为 t 线程要频繁从主内存中读取 run 的值，JIT 编译器会将 run 的值缓存至自己工作内存中的高速缓存中，减少对主存中 run 的访问，提高效率

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668071712486-aa94ca1f-9091-4f01-bd3f-2375aa304048.png)

3. 1 秒之后，main 线程修改了 run 的值，并同步至主存，而 t 是从自己工作内存中的高速缓存中读取这个变量的值，结果永远是旧值

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668071746927-c8ccbfe3-486a-4a4e-9db2-b631f810801b.png)
...
如果在前面示例的死循环中加入 System.out.println() 会发现即使不加 volatile 修饰符，线程 t 也能正确看到对 run 变量的修改了，想一想为什么？因为println方法添加有synchronized关键字
...
这种情况下是：线程2 执行 ready = true，切换到线程1，进入 if 分支，相加为 0，再切回线程2 执行num = 2 ，就是说先执行的是ready = true

相信很多人已经晕了 😵😵?

这种现象叫做指令重排，是 JIT 编译器在运行时的一些优化，这个现象需要通过大量测试才能复现：

借助 java 并发压测工具 jcstress测试
...
可以看到，至于是先执行 i 还是 先执行 j ，对最终的结果不会产生影响。所以，上面代码真正执行时，既可以是i先执行或者是j先执行
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668074048082-46a0dd66-5adc-4c53-aa5f-d1681e2175bc.png)

其中 4 7 两步的顺序不是固定的，也许 jvm 会优化为：先将引用地址赋值给 INSTANCE 变量后，再执行构造方法，如果两个线程 t1，t2 按如下时间序列执行：

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668074072053-1443219e-67fe-447a-a480-195a785229e4.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668079268263-bf267580-af5c-40f3-bb24-680c6b1625f8.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668079338707-c3a1a758-eb95-4d01-93af-d64b339ae625.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668079415682-6e93e601-8238-4c93-ad66-b578735aea67.png)

自旋重试失败的情况

![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668079438627-cf28076e-edce-4687-af17-b05039afd80d.png)
...
![](https://cdn.nlark.com/yuque/0/2022/png/29046015/1668079556498-5df21ba3-2297-4bd9-b513-d40cc454af7d.png)
